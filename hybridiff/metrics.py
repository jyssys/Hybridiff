...
def get_gpu_memory():
    """Returns a dictionary of GPU memory usage per device."""
    ...
    for i in range(torch.cuda.device_count()):
        ...
        memory_max_allocated = torch.cuda.max_memory_allocated(i) / 1024**2  # Convert to MB
        
        memory_usage[f'GPU {i}'] = {
            'Allocated (MB)': memory_allocated,
            'Reserved (MB)': memory_reserved,
            'Max Allocated (MB)': memory_max_allocated
        }
    return memory_usage

def memory_callback(step, timestep, latents):
    mem = get_gpu_memory()
    print(f"[Step {step}, Timestep {timestep}] GPU Memory: {mem}")
