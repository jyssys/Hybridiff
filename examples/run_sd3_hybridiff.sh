...
LOG_DIR="/Hybrid-Diffusion/logs/sd3-hybridiff"
OUTPUT_FILE="${LOG_DIR}/logging-node-${MODEL_N}-stride-${STRIDE}-1024.log"

# Run the command
python -m torch.distributed.run --nproc_per_node=${GPU_COUNT} --master_port=29500 --run_path examples/run_sd3_hybridiff.py \
