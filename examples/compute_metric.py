...
class MultiImageDataset(Dataset):
    def __init__(self, root0, root1, is_gt=False, max_length=None):
        super().__init__()
        self.root0 = root0
        self.root1 = root1
        ...
        # Cut to the shorter of the two lists
        min_length = min(len(self.image_names0), len(self.image_names1))
        if max_length is not None:
            min_length = min(min_length, max_length)
            
        self.image_names0 = self.image_names0[:min_length]
        self.image_names1 = self.image_names1[:min_length]
        
        self.is_gt = is_gt
        assert len(self.image_names0) == len(self.image_names1), "The number of images in the two directories must be the same."
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_length", type=int, default=5000, help="Maximum number of images to load")
    ...
    dataset = MultiImageDataset(args.input_root0, args.input_root1, is_gt=args.is_gt, max_length=args.max_length)
